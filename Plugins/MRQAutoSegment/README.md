...
所有分段都写进你在面板上填的那个**输出目录**，不再每段一个文件夹。区分靠文件名里的帧范围：
...
要拼成一个完整视频，按文件名里的帧范围排序接起来即可 —— `ffmpeg -f concat` 或任何剪辑
软件都行，不用重新编码。
...
每个任务都指向**你自己选的**那个序列，插件不复制、不新建、不改动它。帧范围、文件名、输出
文件夹全部写在任务的 Basic 配置上。

曾经为了绕过"每段都渲染同一段"而给每段生成过一份带帧范围的关卡序列（`/Game/MRQAutoSegment/Segments/`）。
那个目录现在不会再产生了：**帧范围从头到尾都是对的**，真正的原因是 MMD 动作在渲染时没有跟着
序列时间走——见 [`MMDSequencerPreview`](../MMDSequencerPreview) 的 README。既然根因不在范围上，
那些副本就只是没人需要的资产，删掉了。
...
5. **帧范围正确 ≠ 画面正确。** 如果角色的动画不是由 Sequencer 轨道驱动的（比如 MMD 的 VMD
   挂在骨骼网格组件上自己播），那么每个任务都会从头播一遍动作，**每段渲染出来都一样**——
   看起来就像"帧范围没生效"。实测过：两段（`0000-0354` / `0355-0709`）逐帧平均像素差只有
   `0.11/255`，但**段内**首尾帧差有 `10/255`。遇到这个现象先去看
   [`MMDSequencerPreview`](../MMDSequencerPreview) 有没有在渲染时同步。
