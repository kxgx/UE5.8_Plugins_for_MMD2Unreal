...
2. **参数** —— 序列、输出目录、文件名格式、输出格式、分辨率、每像素字节、采样数、帧范围、分段方式、使用上限
...
| 参数 | 默认 | 含义 |
|---|---|---|
| **空间采样** | **8** | 每输出帧的抗锯齿采样数。8 是画质与耗时的常用平衡点 |
| 时间采样 | 1 | 跨时间的子采样,用于运动模糊等 |

**这两个值不参与分段计算** —— 它们只落到生成的任务上,所以改动它们不会重算分段。

取消「写入任务」的勾选,生成的合成就完全不动采样数,交给图自己决定。
(注意:勾选时插件会让任务使用延迟渲染器,因为采样数是延迟渲染器的参数;
如果你的图用的是路径追踪,请取消勾选。)
