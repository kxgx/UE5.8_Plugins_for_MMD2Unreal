...
每个任务都指向**你自己选的**那个序列，插件不会复制、不会新建、不会改动它。

曾经试过"给每段复制一份序列，把帧范围写进复制品"，这条路是错的：复制出来的序列
**内容完全一样**，渲染队列会因此**一直重复第一个任务**，表现就是每段都渲染同一段画面。
现在帧范围只写在任务的 Basic 配置上，序列始终是原来那一个。
...
4. **分辨率只能用项目里已有的命名分辨率。** 面板会写到任务上的分辨率必须是
